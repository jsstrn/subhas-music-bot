import { Album } from "./schema";

export const albums: Album[] = [
  {
    id: "589d0e63-4deb-4c3c-891f-151d9876149e",
    title: "Tabula Rasa",
    artist: { name: "Subhas" },
    description: "This is part one of a two-part sophomore album by Subhas",
    price: 2000,
    lyrics: "https://genius.com/albums/Subhas/Tabula-rasa",
    cover:
      "https://images.genius.com/a9fa6c9b468a68a781a9ca4748f9dc84.1000x1000x1.png",
    credits: [
      { role: "Production", name: "FAUXE" },
      { role: "Vocals", name: "FAUXE" },
      { role: "Tabla", name: "Sana Shajip Komar" },
      { role: "Flute", name: "Islam Robiul" },
      { role: "Additional Vocals", name: "JQ" },
      { role: "Vocal Engineering", name: "FAUXE" },
      { role: "Mixing", name: "FAUXE" },
      { role: "Mastering", name: "FAUXE" },
    ],
    tracks: [
      {
        id: "974524d1-83da-4035-ba07-32062750d564",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "MALABAR",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
          {
            id: "https://cdn.pixabay.com/download/audio/2020/10/11/audio_fe4d3bcac9.mp3?filename=cali.mp3",
            name: "song.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "song",
          },
        ],
      },
      {
        id: "f6b7097c-bd81-40fd-abd4-046265f89bc3",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "WHOSAID?",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
      {
        id: "75c2ba35-b603-4340-8e71-464686246e96",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "MEET THE PEOPLE",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
      {
        id: "8f6e909f-299c-4f32-91e4-dc19b14c4051",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "BHASA",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
      {
        id: "4ef881aa-6bfe-4b34-b0f8-1db2d247f183",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "LONG2BEFREE",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
      {
        id: "d1313fe6-8b1a-4d94-90df-31f88670a464",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "WHITE COLLAR CRIMES",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
      {
        id: "abc7f443-cb8c-473d-801e-ba1207868078",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "UTOPIA (ft. Migrants Band Singapore)",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
      {
        id: "133291b4-48a2-4b4e-bc9d-b90c23be5208",
        albumId: "589d0e63-4deb-4c3c-891f-151d9876149e",
        title: "HEART OF GOLD",
        files: [
          {
            id: "https://cdn.pixabay.com/download/audio/2021/02/08/audio_ecc4386888.mp3?filename=sample.mp3",
            name: "preview.mp3",
            source: "web",
            mime: "audio/mpeg",
            category: "preview",
          },
        ],
      },
    ],
  },
];
